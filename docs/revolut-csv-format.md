...
Before any classification, underscores in the `Type` value are folded to spaces
(then trimmed/lowercased/whitespace-collapsed), so an underscore variant of ANY
type (`STOCK_SPLIT`, `BUY_-_MARKET`, `TRANSFER_FROM_REVOLUT_...`, not just the
ignored `CUSTODY_FEE`) classifies identically to its spaced form. The fold sits
at the top of `classifyType`, ahead of every branch below.
...
| **exactly** `CASH TOP-UP`, `CASH WITHDRAWAL`, `CUSTODY FEE`, `CUSTODY FEE REVERSAL`, or matching `TRANSFER FROM REVOLUT .. TO REVOLUT ..` (the global underscore fold above covers variants like `CUSTODY_FEE`) | ignored | Non-taxable cash movements, account fees + fee refunds, and the UK->EU custodian migration ($0 bookkeeping). Anchored exactly (SUGGESTIONS S14): a bare-substring ignore set would let a never-seen income type such as `MERGER - CASH` vanish silently, and an in-kind transfer from an EXTERNAL broker (cost-basis information we cannot model) must warn, not disappear. Every anchored shape is evidenced in a real export: five in the dickwolff sample, `CUSTODY FEE REVERSAL` in a real revoprofit fixture, `CUSTODY_FEE` in a real stockary statement (PR #270 review survey). |
